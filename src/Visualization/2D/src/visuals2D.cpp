...
{
	glm::ivec2 screenSize = glm::vec2(window.getScreenWidth(), window.getScreenHeight());
...
void Visuals2D::mouseButtonCallback(int button, int action, int mods)
{
	if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS)
	{
		for (int p = 0; p < obstacles.size(); p++)
		{
			...
			{
				if (fabs(mPos.x - obstacles[p].position.x) < obstacles[p].size.x * 0.5f && fabs(mPos.y - obstacles[p].position.y) < obstacles[p].size.y * 0.5f)
				{
					selectedObstacle = p;
...
Visuals2D::Visuals2D(renderer::WindowManager& window, SceneConfig config)
	: GfxInterface(config), window(window)
{
	...
	particleProgram = std::make_shared<renderer::ShaderProgram>("shaders/visualization/2D/particles.vert", "shaders/visualization/2D/basic.frag");
	basic2DProgram = std::make_shared<renderer::ShaderProgram>("shaders/visualization/2D/basic.vert", "shaders/visualization/2D/basic.frag");
	...
	circualrObstacleGfx = std::make_unique<renderer::Object2D<renderer::Circle>>(std::make_shared<renderer::Circle>(60), basic2DProgram);
	boxObstacleGfx = std::make_unique<renderer::Object2D<renderer::Square>>(std::make_shared<renderer::Square>(), basic2DProgram);

	grid2D = std::make_unique<Grid2D>(config.simSize, config.simCenter, config.gridResolution.x, config.gridResolution.y, 1.0f);

	camera->addProgram({ particleProgram, basic2DProgram, grid2D->shaderProgram });
	camera->setUniformsForAllPrograms();

	mouseCallbackNum = window.mouseCallback.onCallback([this](double x, double y) { mouseCallback(x, y); });
	scrollCallbackNum = window.scrollCallback.onCallback([this](double x, double y) { scrollCallback(x, y); });
	mouseButtonCallbackNum = window.mouseButtonCallback.onCallback([this](int a, int b, int c) { mouseButtonCallback(a, b, c); });
}
...
void Visuals2D::handleUserInteractions()
{
	...
	{
		obstacles.push_back(Obstacle(glm::vec3((float)registry["obs.obstacle_x"], (float)registry["obs.obstacle_y"], 0), sceneConfig.simCenter, (glm::vec3)registry["obs.color"]));
...
Visuals2D::~Visuals2D()
{
	...
	window.mouseCallback.removeCallbackFunction(mouseCallbackNum);
	window.mouseButtonCallback.removeCallbackFunction(mouseButtonCallbackNum);
	window.scrollCallback.removeCallbackFunction(scrollCallbackNum);
}
