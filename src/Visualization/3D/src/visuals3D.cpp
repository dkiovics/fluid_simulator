...
void Visuals3D::mouseCallback(double x, double y)
{
	...
	glm::ivec2 screenSize(window.getScreenWidth(), window.getScreenHeight());
...
Visuals3D::Visuals3D(renderer::WindowManager& window, SceneConfig config)
	: GfxInterface(config), window(window)
{
	...
	shaderProgramNotTextured = std::make_shared<renderer::ShaderProgram>("shaders/visualization/3D/object.vert", "shaders/visualization/3D/object_not_textured.frag");
	shaderProgramTextured = std::make_shared<renderer::ShaderProgram>("shaders/visualization/3D/object.vert", "shaders/visualization/3D/object_textured.frag");
	fullScreenShader = std::make_shared<renderer::ShaderProgram>("shaders/utils/fullScreen.vert", "shaders/utils/fullScreen.frag");

	fullScreenQuad = std::make_shared<renderer::Square>();
	renderer::render_target_ptr fullScreenTexture = renderer::make_render_target(screenSize.x, screenSize.y, GL_NEAREST, GL_NEAREST, GL_RGBA8, GL_RGBA, GL_UNSIGNED_BYTE);
	...
	keyCallbackId = window.keyCallback.onCallback([this](int a, int b, int c, int d) { keyCallback(a, b, c, d); });
	mouseCallbackId = window.mouseCallback.onCallback([this](double a, double b) { mouseCallback(a, b); });
	mouseButtonCallbackId = window.mouseButtonCallback.onCallback([this](int a, int b, int c) { mouseButtonCallback(a, b, c); });
	scrollCallbackId = window.scrollCallback.onCallback([this](double a, double b) { scrollCallback(a, b); });
	framebufferSizeCallbackId = window.framebufferSizeCallback.onCallback([this](int a, int b) { screenResolutionChanged(a, b); });
	...
	auto floorTexture = std::make_shared<renderer::ColorTexture>("textures/tiles.jpg", GL_LINEAR_MIPMAP_LINEAR, GL_LINEAR, true);
	...
	boxGfx = std::make_unique<renderer::Object3D<renderer::Cube>>(std::make_shared<renderer::Cube>(), shaderProgramNotTextured);
	sphereGfx = std::make_unique<renderer::Object3D<renderer::Sphere>>(std::make_shared<renderer::Sphere>(80), shaderProgramNotTextured);
	boxGfx->shininess = 80;
...
//void Visuals3D::addObstacle(std::unique_ptr<renderer::Object3D<renderer::Geometry>> obstacleGfx, std::unique_ptr<genericfsim::manager::Obstacle> obstacle, glm::dvec3 size)
//{
...
static glm::vec3 findIntersection(const glm::vec3& planePoint, const glm::vec3& planeNormal, const glm::vec3& linePoint, const glm::vec3& lineDirection)
{
...
static float findIntersection(const glm::vec3& sphereCenter, const float sphereRadius, const glm::vec3& linePoint, const glm::vec3& lineDirection)
{
		...
		else if (o.type == Obstacle::ObstacleType::BOX)
		{
			...
			for (int axis = 0; axis < 3; axis++)
			{
				for (int sign = -1; sign <= 1; sign += 2)
				{
					...
					ignoreAxis[axis] = 1;
					if ((intersection.x >= center.x - size.x * 0.5f && intersection.x <= center.x + size.x * 0.5 || ignoreAxis.x)
						&& (intersection.y >= center.y - size.y * 0.5f && intersection.y <= center.y + size.y * 0.5 || ignoreAxis.y)
						&& (intersection.z >= center.z - size.z * 0.5f && intersection.z <= center.z + size.z * 0.5 || ignoreAxis.z))
					{
						float dist = glm::length(intersection - camPos);
						if (dist < closestPoint)
						{
...
void Visuals3D::screenResolutionChanged(int width, int height)
{
	...
	renderTargetFramebuffer->setSize(screenSize);
	camera->setAspectRatio(float(screenSize.x) / screenSize.y);
	if (fluidRenderer)
	{
		fluidRenderer->setScreenResolution(screenSize);
	}
}
	...
	renderer::fb_ptr fb, renderer::ssbo_ptr<PixelParamData> pixelParamData)
{
	...
	if (pixelParamData != nullptr && pixelParamData->getSize() != screenSize.x * screenSize.y)
...
{
	auto& registry = controls::ControlRegistry::getInstance();
	float dt = (float)window.getLastFrameTime();
	...
	{
		obstacles.push_back(Obstacle((float)registry["obs.obstacle_radius"], sceneConfig.simCenter, (glm::vec3)registry["obs.color"]));
		registry["obs.add_sphere"] = false;
	}
	if (registry["obs.add_rectangle"])
	{
		obstacles.push_back(Obstacle(glm::vec3((float)registry["obs.obstacle_x"], (float)registry["obs.obstacle_y"], (float)registry["obs.obstacle_z"]), sceneConfig.simCenter, (glm::vec3)registry["obs.color"]));
...
Visuals3D::~Visuals3D()
{
	...
	window.mouseCallback.removeCallbackFunction(mouseCallbackId);
	window.mouseButtonCallback.removeCallbackFunction(mouseButtonCallbackId);
	window.scrollCallback.removeCallbackFunction(scrollCallbackId);
	window.keyCallback.removeCallbackFunction(keyCallbackId);
	window.framebufferSizeCallback.removeCallbackFunction(framebufferSizeCallbackId);
}
